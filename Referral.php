...
function gil_send_hubspot_api_details(){
    ...
    update_option( 'gil_list_id', $_POST['gil_list_id'] );
...
function gil_r_send_single_contact_to_hubspot( $args = [] ) {
    ...
    // if contact list is selected add the user
    $gil_list_id = get_option( 'gil_list_id' );

    if( $gil_list_id !== false ){

        gil_addtolist( ['email'=>$args['email']] );
...
function gil_r_get_contact_list(){
    ...
    curl_setopt($ch, CURLOPT_URL, 'https://api.hubapi.com/contacts/v1/lists?count=20' );
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
    curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'GET');

    curl_setopt($ch, CURLOPT_POSTFIELDS, "{\"param0\":\"pradeep\"}");
    ...
    foreach( $Data2["lists"] as $list => $item ){  // echo $item["portalId"] .' => ' . $item["listId"] .' => '. $item["name"] . ' => ' .$item["listType"] . '<br>' ;
        
        array_push($list_list , [
            'port_id' => $item["portalId"],
            'list_id' => $item["listId"],
            'name' => $item["name"],
...
function gil_addtolist( $args = [] ){

    $api_key = get_option('gil_hubspot_key_f');
    $gil_list_id = get_option( 'gil_list_id' );

    $gil_get_user_id = gil_get_user_id( [
        'email'=>$args['email']
    ] ) ;
    ...
    curl_setopt($ch, CURLOPT_URL, 'https://api.hubapi.com/crm/v3/lists/'.$gil_list_id.'/memberships/add-and-remove' );
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
